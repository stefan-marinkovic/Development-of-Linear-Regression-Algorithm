...
class LinearnaRegresija:
    ...
    x_std: float
    alfa=1
    def learn(data,class_atr,lambda_):
        ...
        for iter in range(10000):
            ...
            w=w-LinearnaRegresija.alfa*grad
            MSE=err.T.dot(err)/m
            grad_norm=abs(grad).sum()
            if iter%60==0:
                decay=LinearnaRegresija.alfa/(iter+1)
                LinearnaRegresija.alfa= LinearnaRegresija.alfa * 1/(1 + decay * iter)    
            #alfa= alfa * math.pow(0.5,math.floor((1+iter)/10))
            #exp = np.floor((1 + 10000) / 20)
            #alfa = alfa* (0.5 ** exp)
            print(iter, grad_norm, MSE,LinearnaRegresija.alfa,w)
            #print(alfa)
...
model=LinearnaRegresija.learn(data,'MEDV',0.005)
 
data=pd.read_csv('data/house.csv')
model=LinearnaRegresija.learn(data,'Price',0.005)
#LinearnaRegresija.prediction(model, new_data)

np.logspace(start=0, stop=-3,num=10)
